...
REGISTER_OP("EDiv")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext *c)
      ...
      if (input1_dims == 2){
        ...
        L = c->Value(c->Dim(input1_shape, 2));
        c->set_output(0, c->MakeShape({N, M, L}));
      }
      return Status::OK(); });

REGISTER_OP("EDivGrad")
    .Input("grad: float")
    .Input("input1: float")
    .Input("input2: float")
    .Attr("eid_low: int")
...
class EDivOp : public OpKernel
{
public:
    explicit EDivOp(OpKernelConstruction *context) : OpKernel(context)
    {
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        typedef void (*function)(unsigned long int eid, float *input1, float *input2, int N1, int N2, int *shape1, int *shape2, float *output);
        dlerror();
        function div_kernel = (function)dlsym(lib, "divi");
        const char *dlsym_error = dlerror();
        OP_REQUIRES(context, !dlsym_error, errors::Unknown("loading of dot failed: ", dlsym_error));
        div_kernel(eid_, (float *)input1_flat.data(), (float *)input2_flat.data(), N1, N2, (int *)shape1, (int *)shape2, (float *)output_flat.data());
    };
...
REGISTER_KERNEL_BUILDER(Name("EDiv").Device(DEVICE_CPU), EDivOp);

class EDivGradOp : public OpKernel
{
public:
    explicit EDivGradOp(OpKernelConstruction *context) : OpKernel(context)
    {
        OP_REQUIRES_OK(context, context->GetAttr("eid_low", &eid_low_));
        OP_REQUIRES_OK(context, context->GetAttr("eid_high", &eid_high_));
        OP_REQUIRES_OK(context, context->GetAttr("times", &times_));
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        unsigned long int eid_ = (eid_high_ << 32) + eid_low_;
        typedef void (*function)(unsigned long int eid, float *input1, float *input2, float *grad, int N1, int N2, int *shape1, int *shape2, float *output1, float *output2);
        dlerror();
        function div_grad_kernel = (function)dlsym(lib, "divi_grad");
        const char *dlsym_error = dlerror();
        OP_REQUIRES(context, !dlsym_error, errors::Unknown("loading of dot_grad failed: ", dlsym_error));
        div_grad_kernel(eid_, (float *)input1_flat.data(), (float *)input2_flat.data(), (float *)grad_flat.data(), N1, N2, (int *)shape1, (int *)shape2, (float *)output1_flat.data(), (float *)output2_flat.data());
    };
...
REGISTER_KERNEL_BUILDER(Name("EDivGrad").Device(DEVICE_CPU), EDivGradOp);