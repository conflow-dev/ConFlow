...
class EWhereGreaterOp : public OpKernel
{
public:
    explicit EWhereGreaterOp(OpKernelConstruction *context) : OpKernel(context)
    {
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        dlerror();
        function where_greater_kernel = (function)dlsym(lib, "where_greater");
        const char *dlsym_error = dlerror();
        OP_REQUIRES(context, !dlsym_error, errors::Unknown("loading of where_greater failed: ", dlsym_error));
        where_greater_kernel(eid_, (float *)input1_flat.data(), (float *)input2_flat.data(), (float *)cond1_flat.data(), (float *)cond2_flat.data(), N, n1, n2, (float *)output_flat.data());
    };
...
REGISTER_KERNEL_BUILDER(Name("EWhereGreater").Device(DEVICE_CPU), EWhereGreaterOp);

class EWhereGreaterGradOp : public OpKernel
{
public:
    explicit EWhereGreaterGradOp(OpKernelConstruction *context) : OpKernel(context)
    {
        OP_REQUIRES_OK(context, context->GetAttr("eid_low", &eid_low_));
        OP_REQUIRES_OK(context, context->GetAttr("eid_high", &eid_high_));
        OP_REQUIRES_OK(context, context->GetAttr("times", &times_));
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        dlerror();
        function where_greater_grad_kernel = (function)dlsym(lib, "where_greater_grad");
        const char *dlsym_error = dlerror();
        OP_REQUIRES(context, !dlsym_error, errors::Unknown("loading of where_greater_grad failed: ", dlsym_error));
        where_greater_grad_kernel(eid_, (float *)grad_flat.data(), (float *)input1_flat.data(), (float *)input2_flat.data(), (float *)cond1_flat.data(), (float *)cond2_flat.data(), N, n1, n2, (float *)output1_flat.data(), (float *)output2_flat.data());
    };
...
REGISTER_KERNEL_BUILDER(Name("EWhereGreaterGrad").Device(DEVICE_CPU), EWhereGreaterGradOp);
