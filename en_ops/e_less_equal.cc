...
class ELessEqualOp : public OpKernel
{
public:
    explicit ELessEqualOp(OpKernelConstruction *context) : OpKernel(context)
    {
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        dlerror();
        function lessequal_kernel = (function)dlsym(lib, "lessequal");
        const char *dlsym_error = dlerror();
        OP_REQUIRES(context, !dlsym_error, errors::Unknown("loading of lessequal failed: ", dlsym_error));
        lessequal_kernel(eid_, (float *)input_flat.data(), N, (float *)output_flat.data(), alpha_);
    };
...
REGISTER_KERNEL_BUILDER(Name("ELessEqual").Device(DEVICE_CPU), ELessEqualOp);
